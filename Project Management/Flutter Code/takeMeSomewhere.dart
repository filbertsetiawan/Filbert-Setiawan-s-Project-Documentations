...
/*
step by step supaya paham kodingan ww :')
1. waktu di teken search iconnya, ada 3 engine yang bakal ke generate
- engine Goggle API untuk Transit (sudah aman, sudah bisa keluar polyline sama direction list)
- Engine Bemo aja(List = ListRute)
- Engine Bemo Bus (ListBemoBus)
nah cara kerja engine e adalah :
1. ambil start sama end  lat long direction google api
2/ terus masukin ke engine dapet list
3. list e diambambil coordinat sama stops nya (coords n Stops)
4. dimasukan lagi ke google apis buat dapetin polyline 
5. baru dikeluarin marker + polyline e
*/
class takeMeSomewhere extends StatefulWidget {
  // const takeMeSomewhere({super.key});

  @override
  State<takeMeSomewhere> createState() => _calculatePageState();
}

class _calculatePageState extends State<takeMeSomewhere> {
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          padding: new EdgeInsets.fromLTRB(0, 10, 10, 10),
                          color: Colors.black,
                          onPressed: () async {
                            if (_originController.text == 'Your location') {
                              ...
                              jarak = directions['jarak'].toString();
                              waktuTempuh = directions['durasi'].toString();
                            } else {
                              var directions =
                                  await LocationService().getDirections2(
                            ...
                            // EROR : ak merge punyaku mbek punyamuu, aku gtw seng bener gimana, makae tak merge 2 2 ne
                            setState(() {
                              _showRute = 1;
                              refresh2_1();
                              refresh2_2();
                                              ...
                                              Row(
                                                ...
                                                children: [
                                                  ...
                                                  Row(
                                                    children: [
                                                      Text(
                                                        "${_listWaktuTempuhPerTransport[i]}",
                                                        style: TextStyle(
                                                          fontSize: 14,
                                                          fontWeight:
                                                              FontWeight.bold,
        ...
        child: Row(
          ...
          children: [
            ElevatedButton.icon(
              ...
              onPressed: () async {
                if (_originController.text == 'Your location') {
                  ...
                  _listInfo = [];
                  if (!(tampung2.containsKey("departure_time"))) {
                    int menit = 0;
                    for (String i in _listWaktuTempuhPerTransport) {
                      String cek = "";
                      for (int k = 0; k < i.length; k++) {
                        ...
                        cek += i[k];
                        int jumlah = int.parse(cek);
                        menit += jumlah;
                      }
                    }

                    int jam = menit ~/ 60; // c is now 3
                    menit = menit % 60;
                    ...
                    _listInfo.add(
                        "$hour:$minute"); // prints the current hour and minute in the format "hh:mm"
                    // _listInfo.add("11.00");
                    hour = hour + jam;
                    minute = minute + menit;
                    _listInfo.add("$hour:$minute");
                    _listInfo.add("$jam Hours $menit Minutes");
                  } else {
                    print(tampung2);
                  ...
                  // _listInfo.add("value");
                  if (directions["bayar"] == []) {
                    _listInfo.add(directions["bayar"]["text"]);
                  } else {
                    _listInfo.add("IDR 0.00");
                  }
                  ...
                  for (int i = 0; i < tampung.length; i++) {
                    if (tampung[i]['travel_mode'] == "WALKING") {
                      ...
                      if (tampung[i]['transit_details']['line']['vehicle']
                          ...
                          "Bus") {
                        // BIS
                        if (tampung[i]['transit_details']['line']
                                ['short_name'] ==
                            null) {
                        ...
                        _listPilihIconWithChevron.add("next");
                      } else if (tampung[i]['transit_details']['line']
                              ['vehicle']['name'] ==
                          "Train") {
                        // KERETA
                        _listTransportasi
                  ...
                  var directions = await LocationService().getDirections2(
                    _originController.text,
                    _destinationController.text,
                  );
                  int _polylineIdCounter = 1;
            ...
            ElevatedButton.icon(
              ...
              onPressed: () async {
                _listTransportasi = ["dummy", "dummy", "dummy"];
                _listInstruksi = ["dummy", "dummy", "dummy"];
                _listPilihIcon = ["dummy", "dummy", "dummy"];
                _listPilihIconWithChevron = ["dummy", "dummy", "dummy"];
                _listWaktuTempuhPerTransport = ["dummy", "dummy", "dummy"];
                _listInfo = ["dummy", "dummy", "dummy", "dummy"];
                ...
                //     }, {
                //       'lat': haltee[haltee.length - 1]['arrival_stop']
                //           ['location']['lat'],
  ...
  Future<List> listRute(
      ...
      for (final subroute in route) {
        for (int i = 0; i < subroute.length; i++) {
          if (i < subroute.length - 1) {
            print(subroute[i]);
            print("subroute[i]ttttttttttttt");
            bemos.add(subroute[i][1]);
            stops.add(subroute[i][3]['Stop']);
            coords.add([subroute[i][3]['lat'], subroute[i][3]['long']]);

            stops.add(subroute[i][5]['Stop']);
            coords.add([subroute[i][5]['lat'], subroute[i][5]['long']]);
          } else {
            print(subroute[i]);
            harga += subroute[i];
          }
    ...
    if (latLongHalteAwal == 0 && latLongHalteAkhir == 0) {
      cariPolyline(data, coords, stops, bemos, harga);
    } else {
  ...
  Future cariPolyline(dataAwal, coords, stops, bemos, harga) async {
    _listTransportasi = [];
    _listInstruksi = [];
    _listPilihIcon = [];
    _listPilihIconWithChevron = [];
