<html>
    ...
    <body>
        <?php
            if (!empty($nama)) {
                //cek nama restoran sudah ada atau belum 
                $kalimatquery1 = "SELECT restaurant_name FROM restaurants WHERE restaurant_name = '$nama' LIMIT 1";
                $hasilquery1 = $conn->query($kalimatquery1);

                if ($hasilquery1->num_rows > 0) {
                    echo "Nama restoran sudah ada, silahkan input ulang!";
                }
                else{
                    //insert restoran ke database
                    $kalimatquery = $conn->prepare("INSERT INTO restaurants (id, restaurant_name, category1, category2, street, city, link, phone, open_time, close_time, img) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
